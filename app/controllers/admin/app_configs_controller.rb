class Admin::AppConfigsController < Admin::BaseController
  ...
  def show
    @app_config = Tpd::AppConfig.find(params[:id])
  ...
  def create
    @app_config = Tpd::AppConfig.new(params[:app_config])

    respond_to do |format|
      if @app_config.save
        format.html { redirect_to [:admin,@app_config], notice: 'App config was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @app_config.update_attributes(params[:app_config])
        format.html { redirect_to [:admin,@app_config], notice: 'App config was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_app_configs_url }
      format.json { head :no_content }
    end
  end
end
